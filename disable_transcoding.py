...
def check_file(file: Path) -> tuple[list[str], list[str]]:
    issues = []
    warnings = []
    ...
    if codec == "h264":
        if level > 42:
            issues.append(f"H264 level {level / 10:.1f} — likely needs transcoding")
        elif level > 40:
            warnings.append(f"H264 level {level / 10:.1f} — fine for most clients, may fail on older devices")
...
def scan(paths: list[Path]) -> None:
    ...
    for file in sorted(files):
        issues, warnings = check_file(file)
        if issues:
            flagged += 1
            print(f"\n❌  {file}")
            for issue in issues:
    ...
    print(f"\n{'─' * 60}")
    print(f"  ✅ {ok} OK    ⚠️  {warned} warnings    ❌ {flagged} flagged    {ok + warned + flagged} total")
